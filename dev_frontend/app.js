...
function updateStats() {
    setText('stat-predictions', stats.predictions);
...
async function refreshFeedbackStats() {
    try {
        const res = await fetch('/feedback_stats', { signal: AbortSignal.timeout(5000) });
        if (!res.ok) throw new Error(`Feedback stats error: ${res.status}`);
        const data = await res.json();
        const accuracyPct = data.accuracy === null ? '--' : (data.accuracy * 100).toFixed(1) + '%';
        setText('stat-feedback', data.total);
        setText('stat-accuracy', accuracyPct);
        setText('dev-feedback-total', data.total);
        setText('dev-feedback-correct', data.correct);
        setText('dev-feedback-accuracy', accuracyPct);
    } catch {
        setText('stat-feedback', '--');
...
async function refreshPipelineInfo() {
    try {
        const baseRes = await fetch('/pipeline/baseline_stats', { signal: AbortSignal.timeout(5000) });
        if (baseRes.ok) setJson('dev-baseline-json', await baseRes.json());
    } catch {
        setText('dev-baseline-json', '--');
    }

    try {
        const statusRes = await fetch('/pipeline/status', { signal: AbortSignal.timeout(5000) });
        if (statusRes.ok) setJson('dev-pipeline-status', await statusRes.json());
    } catch {
        setText('dev-pipeline-status', '--');
    }

    try {
        const metricsRes = await fetch('/pipeline/metrics', { signal: AbortSignal.timeout(5000) });
        if (metricsRes.ok) setJson('dev-pipeline-metrics', await metricsRes.json());
    } catch {
        setText('dev-pipeline-metrics', '--');
    }
}
...
refreshFeedbackStats();
refreshPipelineInfo();
setInterval(refreshFeedbackStats, 15000);
